...
const getQrUrl = async ({
  ...
  const url = qrurl.data.data.link as string;
  return url;
};
const orderService = {
  async getOrderHistory({ userId }: { userId: string }) {
  ...
  async createOrder({ userId, cash }: { userId: string; cash: number }) {
  ...
  async finishOrder({ orderId, userId }: { orderId: string; userId: string }) {
